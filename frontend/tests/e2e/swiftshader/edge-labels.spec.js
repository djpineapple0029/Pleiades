...
test('edge labels: ride the line, reveal, hover, declutter, edits', async ({ page }) => {
  ...
  const r = await page.evaluate(async (threeUrl) => {
    ...
    out.flatAboveLine = flat
      ? flat.centre[0] === W / 2 && flat.centre[1] < H / 2 && H / 2 - flat.centre[1] < 25
      : false
  ...
  expect
    .soft(
      r.flat?.angle === 0 &&
        r.flat?.kind === 'edge' &&
        r.flat?.tier === 'edge' &&
        r.flat?.text === 'depends on',
      'a connection name rides its line: level line, level name, centred on the midpoint',
    )
    .toBe(true)
  ...
  expect
    .soft(r.reversedAngle === 0, 'never upside down: the same line drawn the other way reads the same')
    .toBe(true)
  expect
    .soft(
      within(r.upRightAngle, -45, 1e-6) && within(r.downRightAngle, 45, 1e-6),
      'turned to the line: -45 up to the right, +45 down to the right',
    )
    .toBe(true)
  ...
  expect
    .soft(
      r.boxes.level &&
        r.boxes.turned &&
        r.boxes.level.w / r.boxes.level.h > 3 &&
        within(r.boxes.turned.w / r.boxes.turned.h, 1, 0.45),
      'the ink itself is turned: level is wide and flat, 45 degrees is nearly square',
    )
    .toBe(true)
  expect.soft(r.inkInsideRect, 'glyph pixels fall inside the box the entry reserves').toBe(true)
  expect
    .soft(
      r.overRange[0].shown === 1 && r.overRange[1].shown === 1 && r.overRange[3].shown === 0,
      'revealed within range and gone past it (140u for a plain pair)',
    )
    .toBe(true)
  expect
    .soft(
      r.range.core > r.range.plain * 4 && r.coreCarries === 1,
      'a core at one end carries its connection names much further out',
    )
    .toBe(true)
  expect
    .soft(
      r.farGone && r.hovered?.opacity === 1 && r.hovered?.dim === 1,
      'out of range: nothing; under the crosshair: shown, undimmed',
    )
    .toBe(true)
  expect.soft(r.hoverCleared, 'hover cleared: the out-of-range name goes again').toBe(true)
  expect
    .soft(
      r.endOnGone && r.endOnHovered,
      'seen end-on there is no line to write along; the crosshair still names it, level',
    )
    .toBe(true)
  ...
  expect
    .soft(r.clearedGone && r.deletedGone, 'a cleared name and a deleted edge both draw nothing')
    .toBe(true)
  expect.soft(r.loadFadesIn, 'a file reusing an edge id gets the new name, fading in from nothing').toBe(true)
  expect
    .soft(
      r.contested.overlapping === 0 && r.contested.stars >= 2,
      'no label overlaps another, stars and connections together',
    )
    .toBe(true)
  expect
    .soft(r.noLeader, 'a connection name draws one instance and no leader: its line is the leader')
    .toBe(true)
  expect
    .soft(
      JSON.stringify(r.drawsPerFrame) === '[1,1]' && r.labelCalls === 2,
      'names and leaders draw once each a frame; two calls on the label layer',
    )
    .toBe(true)
