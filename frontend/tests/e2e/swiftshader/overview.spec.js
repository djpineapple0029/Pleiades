...
test('overview mode: zoom-to-fit, orbit, handoff to flight', async ({ page }) => {
  ...
  const r = await page
    .evaluate(async (threeUrl) => {
      ...
      const frames = (n, dt = 1 / 60) => {
        for (let i = 0; i < n; i++) overview.update(dt)
      }
      ...
      const drag = (dx, dy, button = 0) => {
        ...
        canvas.dispatchEvent(
          new PointerEvent('pointerdown', { ...opts, button, buttons: button === 0 ? 1 : 2 }),
        )
        canvas.dispatchEvent(
      ...
      physics.start()
      let guard = 0
      while (physics.isRunning && guard++ < 4000) {
        physics.update()
        overview.update(1 / 60)
      }
  ...
  expect
    .soft(r.unlockCalls === 1 && r.lockedAfterEnter === false, 'pointer lock released on the way in')
    .toBe(true)
  expect
    .soft(r.movedPartway && r.notSnapped, 'one frame moves the camera partway, not all the way')
    .toBe(true)
  ...
  expect
    .soft(r.fitWorst > 0.6 && r.fitWorst <= 1, 'the map carries the frame rather than floating in it')
    .toBe(true)
  ...
  // straight overhead (the degenerate lookAt)
  expect
    .soft(r.overheadPolar > 0.1 && r.overheadPolar < Math.PI - 0.1, 'the orbit is held off the pole')
    .toBe(true)
  ...
  expect
    .soft(r.oneDistance, 'one node is framed well clear of its own fade-out')
    .toBeGreaterThan(r.oneRadius * 2.5)
