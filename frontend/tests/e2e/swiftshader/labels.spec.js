...
test('node labels: reveal, tiers, callout, declutter, hover, edits', async ({ page }) => {
  ...
  const r = await page.evaluate(async (threeUrl) => {
    ...
    out.named = rep && {
      tier: rep.tier,
      clean: rep.clean,
      opacity: +rep.opacity.toFixed(3),
      dim: +rep.dim.toFixed(3),
      fontPx: +rep.fontPx.toFixed(2),
    }
    ...
    out.rect = rep && {
      x: +rep.x.toFixed(1),
      y: +rep.y.toFixed(1),
      w: +rep.width.toFixed(1),
      h: +rep.height.toFixed(1),
    }
    ...
    let firstSeen = null
    for (let d = 400; d >= 60; d -= 5) {
      lookAt(0, 0, d, 0, 0, 0)
      run(1, 0.016)
      if (firstSeen === null && shownOf(lone.id)) firstSeen = d
    }
    ...
    lookAt(0, 0, 1.2 * view.radiusOf(lone.id), 0, 0, 0)
    run(20)
    out.tooClose = shownOf(lone.id) === null
    lookAt(0, 0, 100, 0, 0, 200)
    run(20)
    ...
    lookAt(0, 0, 110, 0, 0, 0)
    run(40)
    out.depthPair = [shownOf(front.id)?.fontPx, shownOf(back.id)?.fontPx].map((v) =>
      v ? +v.toFixed(2) : null,
    )
    ...
    for (let i = 0; i < 3; i++) {
      tiers.push(
        graph.addNode({ x: spots[i][0], y: spots[i][1], z: 0, label: ['Core node', 'Hop one', 'Alone'][i] })
          .id,
      )
    }
    ...
    const startOf = (id) => {
      const l = shownOf(id)
      return +Math.hypot(l.leader[0] - gx, l.leader[1] - gy).toFixed(1)
    }
    ...
    out.stacked = {
      ...
      apart:
        stacked.length === 2 &&
        !(
          stacked[0].x < stacked[1].x + stacked[1].width &&
          stacked[1].x < stacked[0].x + stacked[0].width &&
          stacked[0].y < stacked[1].y + stacked[1].height &&
    ...
    for (let i = 0; i < 8; i++)
      crowd.push(
        graph.addNode({ x: (i % 2) * 3, y: i * 0.5, z: -i * 2, label: `a fairly long name number ${i}` }).id,
      )
    view.sync()
    ...
    view.sync()
    lookAt(0, 0, 105, 0, 0, -70)
    run(40)
    const open = shown()
    const openField = discs().filter((d) => d.r >= 3)
    let openCrossings = 0,
      openCleanCrossings = 0
    for (const label of open)
      for (const disc of openField) {
        if (disc.id === label.id || !hits(label, disc)) continue
        openCrossings++
    ...
    out.open = {
      shown: open.length,
      clean: open.filter((l) => l.clean).length,
      cleanCrossings: openCleanCrossings,
    ...
    for (let i = 0; i < 2000; i++)
      line.push(graph.addNode({ x: i * 25, y: 0, z: 0, label: `label number ${i} of the line` }).id)
    view.sync()
    lookAt(0, 0, 120, 0, 0, 0)
    run(20)
    ...
    out.eviction = {
      ...
      maxShown,
      ever: ever.size,
      text: shownOf(line[0])?.text,
      sameRect: shownOf(line[0])?.x === firstRep.x,
    }
  ...
  expect
    .soft(r.layerOnly && r.hiddenEmpty, 'name and leader meshes on LABEL_LAYER only, both hidden when empty')
    .toBe(true)
  expect
    .soft(
      within(r.rule[0], 200, 1e-9) &&
        within(r.rule[1], 304.2, 0.1) &&
        within(r.rule[4], 1576, 1) &&
        r.rule[5] === Infinity &&
        r.rule[6] === Infinity,
      'reveal range: 200 at 1x, ~304 at 1 link, cubic, infinite from 2x',
    )
    .toBe(true)
  expect
    .soft(
      r.onlyNamed && r.namedFull && r.named.clean,
      'unlabelled node draws nothing; a labelled one draws at full strength',
    )
    .toBe(true)
  expect
    .soft(
      r.box &&
        r.box.x0 >= r.rect.x - 1 &&
        r.box.x1 <= r.rect.x + r.rect.w + 1 &&
        r.box.y0 >= r.rect.y - 1 &&
        r.box.y1 <= r.rect.y + r.rect.h + 1 &&
        r.box.x1 - r.box.x0 > r.rect.w - 6,
      'glyph pixels inside the reported rect, and nearly as wide',
    )
    .toBe(true)
  expect
    .soft(
      within(r.callout.start, r.callout.startWant, 0.3) &&
        within(r.callout.diagonal, 1, 0.01) &&
        r.callout.shelfLevel &&
        within(r.callout.textAfterShelf, 5, 0.5) &&
        r.callout.centred < 0.6,
      'callout: leader starts 1.5 radii + 3 px out, runs at 45 degrees to a level shelf, name on the end',
    )
    .toBe(true)
  expect.soft(r.layer0Empty, 'labels are not in the canvas pass').toBe(true)
  expect
    .soft(
      r.reveal.every((p) => within(p.got, p.want, 0.05)),
      'reveal fades in from 200 to 150 units (smoothstep, +-0.05), none past 200',
    )
    .toBe(true)
  expect
    .soft(
      r.firstSeen !== null && r.firstSeen <= 200 && r.firstSeen >= 190,
      'flying straight at a node, its label turns up at ~200 units',
    )
    .toBe(true)
  expect
    .soft(
      r.oneLink[0] === 1 && within(r.oneLink[1], 0.352, 0.03) && r.oneLink[2] === 0,
      'one link does not carry the label further: full at 140, fading at 180, gone at 220',
    )
    .toBe(true)
  expect
    .soft(r.tooClose && r.behind, 'flying through the star hides its label; behind the camera draws nothing')
    .toBe(true)
  expect
    .soft(
      r.sizes.every((p, i) => i === 0 || p.px <= r.sizes[i - 1].px) &&
        r.sizes[0].px >= 28 &&
        r.sizes.at(-1).px === 12 &&
        r.sizes[0].px / r.sizes.at(-1).px > 2.3,
      'size follows distance: biggest close in, smaller every step out, down to a floor',
    )
    .toBe(true)
  expect
    .soft(
      r.sizes[0].dim === 1 && within(r.sizes.at(-1).dim, 0.75, 0.001),
      'distance drains a label too: full close in, down to the core floor far out',
    )
    .toBe(true)
  expect
    .soft(r.depthPair[0], 'of two stars, the nearer one carries the bigger name')
    .toBeGreaterThan(r.depthPair[1] * 1.8)
  expect
    .soft(
      r.tiers[0]?.tier === 'core' &&
        r.tiers[0]?.text === 'CORE NODE' &&
        r.tiers[1]?.tier === 'plain' &&
        r.tiers[1]?.text === 'Hop one' &&
        r.tiers[2]?.tier === 'plain' &&
        r.tiers[2]?.text === 'Alone',
      'tiers: a core in capitals; its neighbour, like a lone node, plain',
    )
    .toBe(true)
  expect
    .soft(
      within(r.coreStart, r.coreStartWant, 1.5) &&
        r.coreStartEasing < r.coreStart &&
        r.coreStartEasing > r.coreStartAfter &&
        r.coreStartAfter < r.coreStart / 2,
      'marked core: the callout moves out with the grown radius, eases back',
    )
    .toBe(true)
  expect
    .soft(
      r.stacked.both && r.stacked.quadrants && r.stacked.apart,
      'two stars on the same spot: both named, callouts turned to different sides',
    )
    .toBe(true)
  expect
    .soft(
      r.crowdPlaced && r.fading && r.fadedOut,
      'in a crowd some names wait, and one that loses its spot fades out',
    )
    .toBe(true)
  expect
    .soft(
      r.hover.shown === 1 &&
        ...
        r.hoverBefore &&
        r.hoverPastRange,
      'hovered: amber, undimmed, wider tracking, and shown past its range',
    )
    .toBe(true)
  expect.soft(r.hoverGone, 'hover cleared: out-of-range label goes').toBe(true)
  expect
    .soft(
      r.editText === 'a considerably longer label' && r.editWider && r.editPixelsWider,
      'edited text shows next frame, no view call, wider in pixels too',
    )
    .toBe(true)
  expect
    .soft(
      r.editCollapsed === 'spaced out' && r.editEmpty,
      'whitespace collapsed to one line; empty label draws nothing',
    )
    .toBe(true)
  expect
    .soft(r.truncated && r.truncatedWidth < 400, 'long label truncated with an ellipsis, width bounded')
    .toBe(true)
  expect.soft(r.deleted, 'deleted node: label gone, both meshes hidden').toBe(true)
  expect
    .soft(r.loadFadesIn && r.loadText === 'new map', 'load reusing an id: new text, fades in from nothing')
    .toBe(true)
  expect
    .soft(
      r.litFull &&
        r.overStar.solid > 30 &&
        r.overStar.same === r.overStar.solid &&
        r.overStar.underLit > r.overStar.solid * 0.5,
      'over a bright star, fully covered glyph pixels keep the exact ink colour',
    )
    .toBe(true)
  expect
    .soft(r.noBloomOutside, 'labels do not bloom: nothing changes outside their ink')
    .toBeLessThanOrEqual(2)
  expect
    .soft(
      JSON.stringify(r.drawsPerFrame) === '[1,1]' && r.labelCalls === 2 && r.cameraRestored,
      'one draw call for the names and one for the leaders; camera layers restored',
    )
    .toBe(true)
  expect
    .soft(
      within(r.ratio.rep[0], r.ratio.rep[1], 0.01) &&
        within(r.ratio.box[0], r.ratio.box[1], 2) &&
        within(r.ratio.centre[0], r.ratio.centre[1], 1),
      'pixel ratio 2: same CSS size and place as at 1',
    )
    .toBe(true)
  ...
  expect
    .soft(
      r.dense.shown > 20 &&
        r.dense.overlapping === 0 &&
        r.dense.cleanCrossings === 0 &&
        r.dense.clean >= r.dense.shown * 0.15 &&
        r.dense.crossings < r.dense.shown * 1.5,
      'dense map: names shown, none overlapping, clean ones clear of every star',
    )
    .toBe(true)
  expect
    .soft(
      r.open.shown >= 8 &&
        r.open.clean >= r.open.shown * 0.8 &&
        r.open.cleanCrossings === 0 &&
        r.open.crossings <= r.open.shown - r.open.clean,
      'an ordinary spread: nearly every name finds a clean spot, and a clean one is off every star',
    )
    .toBe(true)
  expect
    .soft(
      JSON.stringify(r.batches.slice(0, 3)) === '[24,48,72]' &&
        r.batches[4] >= 88 &&
        r.batches[4] === r.batches.at(-1),
      '100 new labels: 24 rasterised a frame, and all that fit are in by the fifth',
    )
    .toBe(true)
  expect
    .soft(
      r.eviction.ever > 400 &&
        r.eviction.worst <= 1 &&
        r.eviction.text === 'label number 0 of the line' &&
        r.eviction.sameRect,
      'after 2000 labels cycle through the atlas, the first reads pixel-identical',
    )
    .toBe(true)
