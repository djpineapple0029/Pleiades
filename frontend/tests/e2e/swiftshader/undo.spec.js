...
test('undo and redo through the keyboard', async ({ page }) => {
  ...
  expect(await t(page, 'hud()')).toBe('map.atlasmap · unsaved · 1 nodes · 0 edges · redo: delete node n1 (1 link)')
  ...
  for (let i = 0; i < 3; i++) { await page.keyboard.press('Control+z'); await settle(page) }
  await page.keyboard.press('Control+z'); await settle(page)
