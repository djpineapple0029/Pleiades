...
test('bloom pipeline and sky: layers, composite, roundness, orientation', async ({ page }) => {
  ...
  expect.soft(r.meshOnStarLayerOnly && r.cameraDefault, 'star mesh is on the star layer only; camera on layer 0').toBe(true)
  expect.soft(Math.max(...r.baseCentre) < 60 && Math.min(...r.singlePassCentre) > 240, 'canvas pass leaves the star out (sky at its centre)').toBe(true)
  expect.soft(r.offVsSingle.worst, 'bloom off reproduces the single-pass frame within 2/255').toBeLessThanOrEqual(2)
  ...
  expect.soft(r.beyondBillboard.slice(0, 3).every((s) => s.on > s.off && Math.abs(s.off - s.sky) <= 3), 'bloom lights the sky beyond the billboard (4.5-8 radii)').toBe(true)
  expect.soft(r.beyondBillboard.every((s, i, xs) => i === 0 || s.on - s.off <= xs[i - 1].on - xs[i - 1].off), 'bloom fades with distance').toBe(true)
  expect.soft(r.roundness.every(({ axis, diag }) => Math.abs(axis - diag) <= 0.25 * Math.max(axis, diag) + 1), 'bloom is round, not boxy (axis vs diagonal within 25%)').toBe(true)
  expect.soft(r.coreStillSaturated, 'core still saturated with bloom').toBe(true)
  expect.soft(r.nonStarsUnbloomed.worst, 'edges, halos, pending line and sky never bloom (frame identical)').toBe(0)
  expect.soft(r.backgroundVsSingle.worst, 'a scene background stays out of the star pass').toBeLessThanOrEqual(2)
  expect.soft(r.resizedVsSingle.worst, 'resize: targets follow the canvas').toBeLessThanOrEqual(2)
  expect.soft(r.ratioVsSingle.worst <= 2 && r.ratioSize[0] === 750, 'pixel ratio change: targets follow the drawing buffer').toBe(true)
  expect.soft(r.bloomCV <= r.starCV * 1.1 && r.bloomMean > 0, 'distant star: bloom varies no more than the star itself under sub-pixel motion').toBe(true)
  expect.soft(r.skyWorstAngleDeg, 'sky samples the world direction (not mirrored), < 3 deg').toBeLessThan(3)
  expect.soft(r.skyStarOffsetPx <= 1.5 && r.skyStarPeak > 300 && r.skyStarMirrorApartPx > 30, 'brightest sky star lands where its direction projects').toBe(true)
  expect.soft(Math.min(...r.bandMeans), 'band brighter than both poles').toBeGreaterThan(Math.max(...r.poleMeans) * 1.15)
  expect.soft(r.dustLit.every((n) => n > 5), 'dust present anywhere in space').toBe(true)
