...
test('clustering becomes a layout, a colour, and survives a reopen', async ({ page }) => {
  ...
  const hueOf0 = (t) => { // mirror of the in-page hue, for the assertion below
    const [r0, g0, b0] = t
  ...
  expect.soft(out.colors.every((list) => new Set(list).size === 1), 'each blob is one colour').toBe(true)
  ...
  expect.soft(out.apart, 'the two clusters are further apart than either is wide').toBeGreaterThan(Math.max(...out.spread) * 2)
  ...
  expect.soft(out.hubPair, 'a single cluster is not compressed inside the link rest length').toBeGreaterThan(out.hubRest - 4)
