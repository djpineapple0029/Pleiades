...
test('edges: width, fog, endpoint fade, hover, drift motes', async ({ page }) => {
  ...
  expect.soft(r.sharedEndpoints, 'motes read the edge endpoint buffer and edge attributes (one upload)').toBe(true)
  expect.soft(r.widthRatios.every((v) => within(v, 1, 0.15)), 'strength follows width x coverage x fog within 15% at 80..1400 units').toBe(true)
  expect.soft(r.widths[0].rows >= 2 && r.widths[0].rows <= 4 && r.widths.slice(3).every((w) => w.rows >= 1 && w.rows <= 2), 'near edge is ~2.5 px, far ones 1-2 px').toBe(true)
  expect.soft(r.fogRangeSmall[0] === 200 && r.fogRangeSmall[1] >= 1000, 'small map: fog never starts inside 200 units').toBe(true)
  expect.soft(r.fogRange.every((v, i) => within(v, r.fogRangeExpected[i], 1)), 'fog range spans the map bounding sphere as seen from the camera').toBe(true)
  ...
  expect.soft(r.farThinPick && r.farThinPickOffset, 'far 1 px edge still pickable, at centre and 5 px off').toBe(true)
  expect.soft(r.settledPick[0] >= 0.95 * r.settledPick[1], 'settled map: edges pickable at their midpoints (>= 95%)').toBe(true)
  expect.soft(r.moteCount >= 3 && r.moteCount <= 7, 'motes drawn along an edge').toBe(true)
  expect.soft(r.undirectedShifts.some((v) => within(v, r.expectShift, 1.2)) && r.undirectedShifts.some((v) => within(v, -r.expectShift, 1.2)) && r.undirectedShifts.every((v) => within(Math.abs(v), r.expectShift, 1.2)), 'undirected: motes drift both ways at DRIFT_SPEED').toBe(true)
  expect.soft(r.directedShifts.length >= 2 && r.directedShifts.every((v) => within(v, r.expectShift, 1.2)), 'directed: every mote drifts from -> to').toBe(true)
  expect.soft(r.longShifts.length >= 3 && r.longShifts.every((v) => within(v, 1.8, 0.5)), "speed is in world units, whatever the length (400-unit edge)").toBe(true)
  expect.soft(r.stretchPhaseSame && r.stretch.length >= 3 && r.stretch.every((v) => Math.abs(v) <= 1.5), 'stretching an edge moves motes continuously, phase untouched').toBe(true)
  expect.soft(r.syncKeepsMotes, 'syncing another edge leaves existing motes in place').toBeLessThanOrEqual(2)
  expect.soft(r.motesAt800 === 0 && r.motesAt100 > 20, 'motes fog out by 800 units, present at 100').toBe(true)
  expect.soft(r.hoverMotesWarm, 'hovered edge motes take the hover colour').toBe(true)
  expect.soft(r.trim.low > 100 && r.trim.high === 0 && r.trim.off === 0, 'edge through the camera plane: trimmed, drawn ahead only, nothing stray').toBe(true)
  expect.soft(within(r.ratioSums[1] / r.ratioSums[0], 2, 0.2), 'widths are CSS px: twice the device-px strength at pixel ratio 2').toBe(true)
