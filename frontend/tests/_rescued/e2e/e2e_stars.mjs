 ...
 * independent pulses, a seamless clock wrap (now covering the ray shimmer too),
 * picking on the node radius rather than the billboard, and everything session
 * 4 already guaranteed — deletes, growth, physics, a file load, halos.
 */
...
check('500 nodes + 499 edges = 3 draw calls (stars, edges, drift)', out.calls500 === 3, `got ${out.calls500}`)
check('exactly one InstancedMesh holding every node', out.instancedCount === 1 && out.instanceCount === 500)
check('no per-node meshes left', out.plainMeshes === 0)
check('core stays saturated through the pulse', out.coreMin >= 240, `${out.coreMin}`)
check('every star visibly pulses', out.pulseRanges.every((d) => d >= 20), JSON.stringify(out.pulseRanges))
check('pulses are not in sync', out.snapshotSpread >= 40, `spread ${out.snapshotSpread}`)
check('rates vary across nodes', out.distinctRates >= 4, `${out.distinctRates}`)
check('every star has its own ray pattern', out.distinctSeeds >= 24, `${out.distinctSeeds}`)
check('tints span blue and warm', out.blueish >= 5 && out.warm >= 2, `blue ${out.blueish}, warm ${out.warm}`)
check('rays shimmer over time', out.shimmerMoves >= 30, `${out.shimmerMoves}`)
check('no jump across the clock wrap (pulse + shimmer, whole frame)', out.wrapJump <= 2, `${out.wrapJump}`)
check('clock is precise after hours of uptime (whole frame)', out.lateDrift <= 2, `${out.lateDrift}`)
check('nothing lit at the billboard edge', out.billboardEdge <= 3, `${out.billboardEdge}`)
check('every star picks as its own node', out.allPicked)
check('pick inside the node radius hits', out.pickInsideRadius)
check('pick in the rays, outside the radius, misses', out.pickInRays === null && out.pickInRaysLit > 10, `${out.pickInRays} lit ${out.pickInRaysLit}`)
check('deleted node no longer picks or draws', out.victimGone && out.victimPixel.every((v) => v <= 3), JSON.stringify(out.victimPixel))
check('survivors still pick correctly after swap-remove', out.survivorsPicked)
check('node moved into the hole keeps its star', out.movedKeepsColor && out.movedKeepsStar)
check('count drops on delete', out.countAfterDelete === 24)
check('node moved outside the old bounds is pickable', out.farPicked)
check('node moved outside the old bounds is not culled', out.farNotCulled, JSON.stringify(out.farPixel))
check('off-screen centre, on-screen rays: not culled', out.edgeStarOffscreen && out.edgeStarLit > 10, `${out.edgeStarOffscreen} ${out.edgeStarLit}`)
check('orbiting 20° changes how a star looks', out.orbitChange > 3, `${out.orbitChange.toFixed(2)}`)
check('rolling the camera turns the rays with the world', out.rollVsRotated < out.rollVsUnrotated * 0.35, `rotated ${out.rollVsRotated.toFixed(2)} vs unrotated ${out.rollVsUnrotated.toFixed(2)}`)
check('growth keeps one mesh, replaced', out.growthOneMesh && out.growthReplaced && out.growthCapacity === 2048 && out.growthCount === 1121, `${out.growthCapacity}/${out.growthCount}`)
check('render order survives reallocation', out.renderOrderKept)
check('grown nodes pickable', out.grownPicked)
check('still 1 draw call for nodes after growth (no edges)', out.callsAfterGrowth === 1, `${out.callsAfterGrowth}`)
check('star survives reallocation', out.starPreservedAcrossGrowth)
check('physics settles', out.settled)
check('every node pickable after a settle', out.pickedAfterSettle === 60, `${out.pickedAfterSettle}/60`)
check('file load renders onto instances', out.loadedCount === 2 && out.loadedLeft && out.loadedEdgeMid === 'edge', JSON.stringify([out.loadedCount, out.loadedLeft, out.loadedEdgeMid]))
check('edge does not darken the core it enters', out.coreOverEdge.every((v) => v >= 240), JSON.stringify(out.coreOverEdge))
check('hover halo sits on, follows, and hides', out.haloOnNode && out.haloFollows && out.haloHidden)
check('3000 nodes: 1 draw call', out.calls3000 === 1, `${out.calls3000}`)
check('3000 nodes: syncNodes under 2 ms', out.syncMs3000 < 2, `${out.syncMs3000.toFixed(3)} ms`)
check('3000 nodes: raycast under 1 ms', out.raycastMs3000 < 1, `${out.raycastMs3000.toFixed(3)} ms`)
