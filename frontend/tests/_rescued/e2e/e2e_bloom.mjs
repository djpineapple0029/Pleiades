/**
 * Session 6: the bloom pipeline and the sky, against the real modules.
 * Deterministic SwiftShader, so pixel comparisons are exact.
 */
...
const r = await page.evaluate(async () => {
  ...
  const baseOnly = () => { renderer.setRenderTarget(null); renderer.render(scene, camera); return read() }
  const through = (pipeline) => { pipeline.render(); return read() }
  const px = (buf, x, y) => { const i = (Math.round(y) * W + Math.round(x)) * 4; return [buf[i], buf[i + 1], buf[i + 2]] }
  ...
  out.stateRestored = camera.layers.mask === 1 && renderer.autoClear === true &&
    cc.getHex() === new THREE.Color(VOID_COLOR).getHex() && renderer.getClearAlpha() === 1 &&
    renderer.getRenderTarget() === null && scene.background === null
  ...
  out.beyondBillboard = [4.5, 6, 8, 12].map((radii) => ({
    radii, off: mean(ring(off, radii)), on: mean(ring(on, radii)), sky: mean(ring(base, radii)),
  }))
  // Axis vs diagonal at 8 radii (well beyond the rays): a box blur would be
  // brighter on the diagonals.
  const axisVsDiag = (radii) => {
    const d = radii * NODE_RADIUS * pxPerUnit
    const at = (ang) => lum(px(on, cx + d * Math.cos(ang), cy + d * Math.sin(ang))) - lum(px(off, cx + d * Math.cos(ang), cy + d * Math.sin(ang)))
    return { axis: mean([0, 1, 2, 3].map((k) => at((k * Math.PI) / 2))), diag: mean([0, 1, 2, 3].map((k) => at((k * Math.PI) / 2 + Math.PI / 4))) }
  }
  ...
  mesh().visible = true
  view.setHover(null); view.setSource(null); view.setPending(null)
  ...
  graph.load({ nodes: [{ id: 'far', label: '', notes: '', links: [], x: 0, y: 0, z: 0, cluster_color_id: null, is_core: false }], edges: [] })
  view.sync()
  const dist = 1400
  const unitPx = (() => { look(0, 0, 0, dist); return screenOf(1, 0, 0)[0] - screenOf(0, 0, 0)[0] })()
  const lin = Array.from({ length: 256 }, (_, c) => { const v = c / 255; return v <= 0.04045 ? v / 12.92 : ((v + 0.055) / 1.055) ** 2.4 })
  const bloomSums = [], starSums = []
  for (let s = 0; s < 17; s++) {
    const node = graph.getNode('far')
    node.x = (s / 8) / unitPx
    view.syncNodes()
    look(0, 0, 0, dist)
    const withB = through(bloom), without = through(noBloom), sky0 = (mesh().visible = false, baseOnly())
    mesh().visible = true
    ...
    bloomSums.push(bs); starSums.push(ss)
  }
  const cv = (xs) => { const m = mean(xs); return Math.sqrt(mean(xs.map((x) => (x - m) ** 2))) / m }
  ...
  graph.load({ nodes: [], edges: [] }); view.sync()
  dust.visible = false
  const skyStars = sky.object.getObjectByName('sky-stars')
  skyStars.visible = false
  const probe = new THREE.WebGLCubeRenderTarget(64, {
    colorSpace: THREE.SRGBColorSpace, generateMipmaps: false, minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter,
  })
  const probeScene = new THREE.Scene()
  probeScene.add(new THREE.Mesh(new THREE.BoxGeometry(2, 2, 2), new THREE.ShaderMaterial({
    vertexShader: 'varying vec3 vD; void main(){ vD = position; gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0); }',
    fragmentShader: 'varying vec3 vD; void main(){ gl_FragColor = vec4(normalize(vD) * 0.5 + 0.5, 1.0); }',
    side: THREE.BackSide,
  })))
  ...
  const decode = (c) => { const s = c / 255; return s <= 0.04045 ? s / 12.92 : ((s + 0.055) / 1.055) ** 2.4 }
  let worstAngle = 0
  const raycaster = new THREE.Raycaster()
  for (const [rx, ry, rz] of [[0, 0, 0], [0.6, 1.9, 0.3], [-0.9, -2.7, 0], [1.2, 0.4, -0.8]]) {
    camera.position.set(123, -45, 678)
    camera.rotation.set(rx, ry, rz)
    camera.updateMatrixWorld(true)
    const frame = baseOnly()
    for (const [sx, sy] of [[0.5, 0.5], [0.1, 0.15], [0.9, 0.2], [0.2, 0.85], [0.85, 0.9]]) {
  ...
  let brightest = 0
  for (let i = 1; i < lights.count; i++) if (lights.getX(i) + lights.getY(i) > lights.getX(brightest) + lights.getY(brightest)) brightest = i
  ...
  let best = { l: -1 }
  for (let y = Math.round(ey) - 8; y <= Math.round(ey) + 8; y++) for (let x = Math.round(ex) - 8; x <= Math.round(ex) + 8; x++) {
    const l = lum(px(starFrame, x, y)); if (l > best.l) best = { l, x, y }
  }
  ...
  skyStars.visible = false
  const meanFrame = () => { const f = baseOnly(); let s = 0; for (let i = 0; i < f.length; i += 4) s += f[i] + f[i + 1] + f[i + 2]; return s / (f.length / 4) }
  const N = new THREE.Vector3(0.35, 0.85, 0.4).normalize()
  camera.position.set(0, 0, 0)
  camera.lookAt(N); camera.updateMatrixWorld(true)
  const pole = meanFrame()
  camera.lookAt(N.clone().negate()); camera.updateMatrixWorld(true)
  ...
  const bandMeans = [0, 1, 2, 3].map((k) => {
    ...
    camera.lookAt(d); camera.updateMatrixWorld(true)
    return meanFrame()
  })
  ...
  sky.object.visible = false
  const dustAt = (x, y, z) => {
    camera.position.set(x, y, z); camera.rotation.set(0, 0, 0); camera.updateMatrixWorld(true)
    const f = baseOnly()
    const bg = new THREE.Color(VOID_COLOR)
    let lit = 0
  ...
  bloom.dispose(); noBloom.dispose(); sky.dispose()
  return out
})

console.log(JSON.stringify(r, null, 1))
check('star mesh is on the star layer only; camera on layer 0', r.meshOnStarLayerOnly && r.cameraDefault)
check('canvas pass leaves the star out (sky at its centre)', Math.max(...r.baseCentre) < 60 && Math.min(...r.singlePassCentre) > 240, JSON.stringify([r.baseCentre, r.singlePassCentre]))
check('bloom off reproduces the single-pass frame within 2/255', r.offVsSingle.worst <= 2, JSON.stringify(r.offVsSingle))
check('stars drawn exactly once per frame', r.starDrawsPerFrame === 1, r.starDrawsPerFrame)
check('renderer, camera and scene state restored after a frame', r.stateRestored)
check('bloom lights the sky beyond the billboard (4.5-8 radii)', r.beyondBillboard.slice(0, 3).every((s) => s.on > s.off && Math.abs(s.off - s.sky) <= 3), JSON.stringify(r.beyondBillboard))
check('bloom fades with distance', r.beyondBillboard.every((s, i, xs) => i === 0 || s.on - s.off <= xs[i - 1].on - xs[i - 1].off))
check('bloom is round, not boxy (axis vs diagonal within 25%)', r.roundness.every(({ axis, diag }) => Math.abs(axis - diag) <= 0.25 * Math.max(axis, diag) + 1), JSON.stringify(r.roundness))
check('core still saturated with bloom', r.coreStillSaturated)
check('edges, halos, pending line and sky never bloom (frame identical)', r.nonStarsUnbloomed.worst === 0, JSON.stringify(r.nonStarsUnbloomed))
check('a scene background stays out of the star pass', r.backgroundVsSingle.worst <= 2, JSON.stringify(r.backgroundVsSingle))
check('resize: targets follow the canvas', r.resizedVsSingle.worst <= 2, JSON.stringify(r.resizedVsSingle))
check('pixel ratio change: targets follow the drawing buffer', r.ratioVsSingle.worst <= 2 && r.ratioSize[0] === 750, JSON.stringify([r.ratioSize, r.ratioVsSingle]))
check('distant star: bloom varies no more than the star itself under sub-pixel motion', r.bloomCV <= r.starCV * 1.1 && r.bloomMean > 0, JSON.stringify({ bloomCV: r.bloomCV, starCV: r.starCV, farStarPx: r.farStarPx, bloomMean: r.bloomMean }))
check('sky samples the world direction (not mirrored), < 3 deg', r.skyWorstAngleDeg < 3, r.skyWorstAngleDeg)
check('brightest sky star lands where its direction projects', r.skyStarOffsetPx <= 1.5 && r.skyStarPeak > 300 && r.skyStarMirrorApartPx > 30, JSON.stringify([r.skyStarOffsetPx, r.skyStarPeak, r.skyStarMirrorApartPx]))
check('band brighter than both poles', Math.min(...r.bandMeans) > Math.max(...r.poleMeans) * 1.15, JSON.stringify([r.bandMeans, r.poleMeans]))
check('dust present anywhere in space', r.dustLit.every((n) => n > 5), JSON.stringify(r.dustLit))
check('mesh rebuilt past 256 keeps the star layer', r.grownMeshOnStarLayer)
