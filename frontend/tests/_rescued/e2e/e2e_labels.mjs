/**
 * Session 9: the redesigned node labels, against the real modules.
 * Deterministic SwiftShader. Covers the reveal rule, the three tiers, size and
 * dimming with distance, the callout and its leader, keeping off other stars,
 * declutter, hover, edits/deletes/loads, the atlas, and the draw.
 */
...
check('name and leader meshes on LABEL_LAYER only, both hidden when empty', r.layerOnly && r.hiddenEmpty)
check('reveal range: 200 at 1x, ~304 at 1 link, cubic, infinite from 2x', within(r.rule[0], 200, 1e-9) && within(r.rule[1], 304.2, 0.1) && within(r.rule[4], 1576, 1) && r.rule[5] === Infinity && r.rule[6] === Infinity, JSON.stringify(r.rule))
check('unlabelled node draws nothing; a labelled one draws at full strength', r.onlyNamed && r.namedFull && r.named.clean, JSON.stringify([r.onlyNamed, r.named]))
check('glyph pixels inside the reported rect, and nearly as wide', r.box && r.box.x0 >= r.rect.x - 1 && r.box.x1 <= r.rect.x + r.rect.w + 1 && r.box.y0 >= r.rect.y - 1 && r.box.y1 <= r.rect.y + r.rect.h + 1 && r.box.x1 - r.box.x0 > r.rect.w - 6, JSON.stringify([r.box, r.rect]))
check('callout: leader starts 1.5 radii + 3 px out, runs at 45 degrees to a level shelf, name on the end', within(r.callout.start, r.callout.startWant, 0.3) && within(r.callout.diagonal, 1, 0.01) && r.callout.shelfLevel && within(r.callout.textAfterShelf, 5, 0.5) && r.callout.centred < 0.6, JSON.stringify(r.callout))
check('labels are not in the canvas pass', r.layer0Empty)
check('reveal fades in from 200 to 150 units (smoothstep, +-0.05), none past 200', r.reveal.every((p) => within(p.got, p.want, 0.05)), JSON.stringify(r.reveal))
check('flying straight at a node, its label turns up at ~200 units', r.firstSeen !== null && r.firstSeen <= 200 && r.firstSeen >= 190, r.firstSeen)
check('one link carries the label further: full at 220, fading at 280, gone at 320', r.oneLink[0] === 1 && within(r.oneLink[1], 0.239, 0.03) && r.oneLink[2] === 0, JSON.stringify(r.oneLink))
check('flying through the star hides its label; behind the camera draws nothing', r.tooClose && r.behind)
check('size follows distance: biggest close in, smaller every step out, down to a floor', r.sizes.every((p, i) => i === 0 || p.px <= r.sizes[i - 1].px) && r.sizes[0].px >= 28 && r.sizes.at(-1).px === 12 && r.sizes[0].px / r.sizes.at(-1).px > 2.3, JSON.stringify(r.sizes))
check('distance drains a label too: full close in, down to the core floor far out', r.sizes[0].dim === 1 && within(r.sizes.at(-1).dim, 0.75, 0.001), JSON.stringify(r.sizes.map((p) => p.dim)))
check('of two stars, the nearer one carries the bigger name', r.depthPair[0] > r.depthPair[1] * 1.8, JSON.stringify(r.depthPair))
check('tiers: a core in capitals, its neighbour a landmark, a lone node plain', r.tiers[0]?.tier === 'core' && r.tiers[0]?.text === 'CORE NODE' && r.tiers[1]?.tier === 'landmark' && r.tiers[1]?.text === 'Hop one' && r.tiers[2]?.tier === 'plain' && r.tiers[2]?.text === 'Alone', JSON.stringify(r.tiers))
check('marked core: the callout moves out with the grown radius, eases back', within(r.coreStart, r.coreStartWant, 1.5) && r.coreStartEasing < r.coreStart && r.coreStartEasing > r.coreStartAfter && r.coreStartAfter < r.coreStart / 2, JSON.stringify([r.coreStart, r.coreStartWant, r.coreStartEasing, r.coreStartAfter]))
check('two stars on the same spot: both named, callouts turned to different sides', r.stacked.both && r.stacked.quadrants && r.stacked.apart, JSON.stringify(r.stacked))
check('in a crowd some names wait, and one that loses its spot fades out', r.crowdPlaced && r.fading && r.fadedOut, JSON.stringify([r.crowdPlaced, r.fading, r.fadedOut]))
check('hovered: amber, undimmed, wider tracking, and shown past its range', r.hover.shown === 1 && r.hover.undimmed && r.hover.wider && r.hoverWarm && r.hoverBefore && r.hoverPastRange, JSON.stringify([r.hover, r.hoverWarm, r.hoverBefore, r.hoverPastRange]))
check('hover cleared: out-of-range label goes', r.hoverGone)
check('edited text shows next frame, no view call, wider in pixels too', r.editText === 'a considerably longer label' && r.editWider && r.editPixelsWider)
check('whitespace collapsed to one line; empty label draws nothing', r.editCollapsed === 'spaced out' && r.editEmpty, JSON.stringify([r.editCollapsed, r.editEmpty]))
check('long label truncated with an ellipsis, width bounded', r.truncated && r.truncatedWidth < 400, JSON.stringify([r.truncated, r.truncatedWidth]))
check('deleted node: label gone, both meshes hidden', r.deleted)
check('load reusing an id: new text, fades in from nothing', r.loadFadesIn && r.loadText === 'new map', JSON.stringify([r.loadFadesIn, r.loadText]))
check('over a bright star, fully covered glyph pixels keep the exact ink colour', r.litFull && r.overStar.solid > 30 && r.overStar.same === r.overStar.solid && r.overStar.underLit > r.overStar.solid * 0.5, JSON.stringify([r.litFull, r.overStar]))
check('labels do not bloom: nothing changes outside their ink', r.noBloomOutside <= 2, r.noBloomOutside)
check('one draw call for the names and one for the leaders; camera layers restored', JSON.stringify(r.drawsPerFrame) === '[1,1]' && r.labelCalls === 2 && r.cameraRestored, JSON.stringify([r.drawsPerFrame, r.labelCalls, r.cameraRestored]))
check('pixel ratio 2: same CSS size and place as at 1', within(r.ratio.rep[0], r.ratio.rep[1], 0.01) && within(r.ratio.box[0], r.ratio.box[1], 2) && within(r.ratio.centre[0], r.ratio.centre[1], 1), JSON.stringify(r.ratio))
// 300 nodes in a 400-unit box, seen from 150: stars cover most of the screen,
// so many names can only take a least-bad spot. A clean one never crosses a
// star, and the rest average well under one star each.
check('dense map: names shown, none overlapping, clean ones clear of every star', r.dense.shown > 20 && r.dense.overlapping === 0 && r.dense.cleanCrossings === 0 && r.dense.clean > 10 && r.dense.crossings < r.dense.shown * 1.5, JSON.stringify(r.dense))
// The odd name out takes a least-bad spot and may sit on a star; a clean one
// never does. That is the whole contract of the two passes.
check('an ordinary spread: nearly every name finds a clean spot, and a clean one is off every star', r.open.shown >= 8 && r.open.clean >= r.open.shown * 0.8 && r.open.cleanCrossings === 0 && r.open.crossings <= r.open.shown - r.open.clean, JSON.stringify(r.open))
// Callouts ask for more room than a name under a star did, so in a tight grid
// a few names wait for a spot; what matters here is the rasterising cadence.
check('100 new labels: 24 rasterised a frame, and all that fit are in by the fifth', JSON.stringify(r.batches.slice(0, 3)) === '[24,48,72]' && r.batches[4] >= 88 && r.batches[4] === r.batches.at(-1), JSON.stringify(r.batches))
check('after 2000 labels cycle through the atlas, the first reads pixel-identical', r.eviction.ever > 400 && r.eviction.worst <= 1 && r.eviction.text === 'label number 0 of the line' && r.eviction.sameRect, JSON.stringify(r.eviction))
check('3000 labelled nodes: update < 2 ms/frame', r.update3000 < 2, JSON.stringify([r.update3000, r.update3000Blank]))
