/**
 * Session 7: edge width, depth fog, endpoint fade, hover, and drift motes,
 * against the real modules. Deterministic SwiftShader. Layer-0 renders only,
 * onto black, so the stars (STAR_LAYER) never enter a measurement.
 */
...
const r = await page.evaluate(async () => {
  ...
  const show = ({ edges = true, motes = true }) => { lines().layers.set(edges ? 0 : 7); drift().layers.set(motes ? 0 : 7) }
  ...
  const frame = () => { renderer.setRenderTarget(null); renderer.render(scene, camera); return read() }
  ...
  const band = (buf, x0, x1, yc, half = 8, c = 2) => {
    let s = 0
    for (let x = Math.round(x0); x <= Math.round(x1); x++) for (let y = Math.round(yc) - half; y <= Math.round(yc) + half; y++) s += at(buf, x, y, c)
  ...
  const reset = () => { graph.load({ nodes: [], edges: [] }); physics.reset(); view.sync(); view.setHover(null) }
  const link = (a, b) => {
    const na = graph.addNode({ x: a[0], y: a[1], z: a[2] }), nb = graph.addNode({ x: b[0], y: b[1], z: b[2] })
    return { a: na, b: nb, edge: graph.addEdge(na.id, nb.id) }
  }
  let clock = 10
  const tick = (dt) => { clock += dt; view.update(clock) }
  ...
  out.sharedEndpoints = drift().geometry.getAttribute('instanceStart').data === lines().geometry.getAttribute('instanceStart').data &&
  ...
  const expectLine = (d) => {
    ...
    const drawn = Math.min(2.5, Math.max(1, tw)), cov = Math.sqrt(Math.min(1, tw))
    const near = Math.max(200, d - 100), far = Math.max(d + 100, near + 800) // sphere r = 100 around the origin
  ...
  out.widths = [80, 200, 343, 700, 1400].map((d) => {
    ...
    return { d, sum: band(f, W / 2 - half, W / 2 + half, H / 2), rows: litRows(f, Math.round(W / 2 + 7), H / 2), expect: expectLine(d) }
  })
  const k = out.widths[0].sum / out.widths[0].expect
  out.widthRatios = out.widths.map((w) => +(w.sum / (k * w.expect)).toFixed(3))
  out.fogRangeSmall = (() => { lookAt(0, 0, 150, 0, 0, 0); frame(); return lines().material.uniforms.fogRange.value.toArray() })()
  ...
  out.fogRangeExpected = [Math.max(200, D - R), Math.max(D + R, Math.max(200, D - R) + 800)].map((v) => +v.toFixed(1))
  const measureAt = (y, z) => {
    const d = D - z, [, sy] = screenOf(0, y, z), half = 300 * pxAt(d)
    const tw = pxAt(d), drawn = Math.min(2.5, Math.max(1, tw)), cov = Math.sqrt(Math.min(1, tw))
    return { sum: band(ff, W / 2 - half, W / 2 + half, sy, 4), drawnCov: drawn * cov, fog: 1 - 0.75 * smooth(range[0], range[1], d) }
  }
  const nearSide = measureAt(60, 400), farSide = measureAt(-60, -400)
  ...
  const strengthAt = (buf, x) => { const [sx] = screenOf(x, 0, 0); return band(buf, sx, sx, H / 2, 6) }
  const ref = strengthAt(frame(), 100)
  const rA = view.radiusOf(f1.a.id)
  out.endRadius = rA
  const endFrame = frame()
  out.endProfile = [0.5, 0.9, 1.1, 1.6, 2.0, 2.6, 4, 8].map((m) => ({ m, got: +(strengthAt(endFrame, m * rA) / ref).toFixed(3), want: +smooth(1.1 * rA, 2.4 * rA, m * rA).toFixed(3) }))
  ...
  out.coreEnd = [1.6 * rA, 1.5 * rCore, 2.6 * rCore].map((x) => +(strengthAt(coreFrame, x) / ref).toFixed(3))
  ...
  out.hoverPlain = { red: +band(plain, ...hx, H / 2, 6, 0).toFixed(1), blue: +band(plain, ...hx, H / 2, 6, 2).toFixed(1) }
  out.hoverLit = { red: +band(lit, ...hx, H / 2, 6, 0).toFixed(1), blue: +band(lit, ...hx, H / 2, 6, 2).toFixed(1), rows: litRows(lit, W / 2 + 7, H / 2, 6, 0, 60) }
  ...
  view.syncEdges()
  graph.removeEdge(h1.edge.id); graph.addEdge(h1.a.id, h1.b.id) // hovered id gone; re-added as a new id
  view.syncEdges()
  ...
  lookAt(0, 0, 1400, 0, 0, 0)
  const moved = frame()
  out.hoverSurvivesSync = lines().material.uniforms.hovered.value === 0 && band(moved, ...hx, H / 2, 6, 0) > out.hoverPlain.red * 3
  graph.removeEdge(s2.edge.id); view.syncEdges()
  ...
  const ids = []
  for (let i = 0; i < 120; i++) { ids.push(graph.addNode({ x: (rand() - 0.5) * 300, y: (rand() - 0.5) * 300, z: (rand() - 0.5) * 300 }).id); if (i) graph.addEdge(ids[i], ids[Math.floor(rand() * i)]) }
  view.sync(); physics.start(); let g = 0; while (physics.isRunning && g++ < 4000) physics.update()
  let picked = 0, tried = 0
  for (const edge of graph.edges.values()) {
    const a = graph.getNode(edge.from), b = graph.getNode(edge.to)
  ...
  const motes = (buf) => {
    const prof = []
    for (let x = 0; x < W; x++) { let v = 0; for (let y = H / 2 - 4; y <= H / 2 + 4; y++) v += at(buf, x, y, 2); prof.push(v) }
    // One connected run of lit columns per mote; its intensity centroid.
    const found = []
    for (let x = 0; x < W; ) {
      if (prof[x] < 30) { x++; continue }
      let m = 0, w = 0
      while (x < W && prof[x] >= 30) { m += x * prof[x]; w += prof[x]; x++ }
      found.push(m / w)
    }
    return found
  }
  const shifts = (before, after) => after.map((x) => {
    let best = null
  ...
  tick(0)
  const l0 = frame(); tick(0.1); tick(0.1); const l1 = frame()
  ...
  const x0 = screenOf(0, 0, 0)[0], perUnit = pxAt(240)
  ...
  for (let x = 0; x < bx - 20; x++) for (let y = H / 2 - 6; y <= H / 2 + 6; y++) syncDiff = Math.max(syncDiff, Math.abs(at(quiet, x, y, 2) - at(afterSync, x, y, 2)))
  out.syncKeepsMotes = syncDiff
  // Motes fog out: none at 800 units, some at 100.
  reset()
  link([-100, 0, 0], [100, 0, 0])
  view.sync()
  const litCount = (buf) => { let n = 0; for (let i = 0; i < buf.length; i += 4) if (buf[i] + buf[i + 1] + buf[i + 2] > 0) n++; return n }
  lookAt(0, 0, 800, 0, 0, 0); tick(0)
  ...
  const hm = frame()
  let red = 0, blue = 0
  for (let i = 0; i < hm.length; i += 4) { red += hm[i]; blue += hm[i + 2] }
  ...
  renderer.setPixelRatio(1); W = 640; H = 480
  show({})

  // --- Draw calls and cost -------------------------------------------------------
  reset()
  const many = []
  for (let i = 0; i < 3001; i++) many.push(graph.addNode({ x: (i % 60) * 20, y: Math.floor(i / 60) * 20, z: 0 }).id)
...
check('edges and motes on layer 0 only', r.layers)
check('motes read the edge endpoint buffer and edge attributes (one upload)', r.sharedEndpoints)
check('strength follows width x coverage x fog within 15% at 80..1400 units', r.widthRatios.every((v) => within(v, 1, 0.15)), JSON.stringify(r.widthRatios))
check('near edge is ~2.5 px, far ones 1-2 px', r.widths[0].rows >= 2 && r.widths[0].rows <= 4 && r.widths.slice(3).every((w) => w.rows >= 1 && w.rows <= 2), JSON.stringify(r.widths.map((w) => w.rows)))
check('small map: fog never starts inside 200 units', r.fogRangeSmall[0] === 200 && r.fogRangeSmall[1] >= 1000, JSON.stringify(r.fogRangeSmall))
check('fog range spans the map bounding sphere as seen from the camera', r.fogRange.every((v, i) => within(v, r.fogRangeExpected[i], 1)), JSON.stringify([r.fogRange, r.fogRangeExpected]))
check('map seen from 2600+ units: near side barely fogged (sphere is loose)', within(r.fogNearSide, r.fogPredicted[0], 0.12) && r.fogNearSide > 0.8, JSON.stringify([r.fogNearSide, r.fogPredicted]))
check('map seen from outside: far side at the fog floor', within(r.fogFarSide, r.fogPredicted[1], 0.08), JSON.stringify([r.fogFarSide, r.fogPredicted]))
check('edge fades out inside its end node (smoothstep 1.1r..2.4r, +-0.12)', r.endProfile.every((p) => within(p.got, p.want, 0.12)), JSON.stringify(r.endProfile))
check('core end: fade grows with the eased radius', r.coreRadius > 14 && r.coreEnd[0] < 0.05 && r.coreEnd[1] < 0.35 && r.coreEnd[2] > 0.9, JSON.stringify([r.coreRadius, r.coreEnd]))
check('hovered far edge: warm, wider and far stronger', r.hoverLit.red > r.hoverLit.blue && r.hoverLit.red > r.hoverPlain.red * 5 && r.hoverLit.rows >= 3, JSON.stringify([r.hoverPlain, r.hoverLit]))
check('hover follows its edge through a renumbering sync', r.hoverSurvivesSync)
check('hover clears when its edge is removed', r.hoverClearedWithEdge && r.hoverGone)
check('far 1 px edge still pickable, at centre and 5 px off', r.farThinPick && r.farThinPickOffset)
check('settled map: edges pickable at their midpoints (>= 95%)', r.settledPick[0] >= 0.95 * r.settledPick[1], JSON.stringify(r.settledPick))
check('motes drawn along an edge', r.moteCount >= 3 && r.moteCount <= 7, r.moteCount)
check('undirected: motes drift both ways at DRIFT_SPEED', r.undirectedShifts.some((v) => within(v, r.expectShift, 1.2)) && r.undirectedShifts.some((v) => within(v, -r.expectShift, 1.2)) && r.undirectedShifts.every((v) => within(Math.abs(v), r.expectShift, 1.2)), JSON.stringify([r.undirectedShifts, r.expectShift]))
check('directed: every mote drifts from -> to', r.directedShifts.length >= 2 && r.directedShifts.every((v) => within(v, r.expectShift, 1.2)), JSON.stringify(r.directedShifts))
check('speed is in world units, whatever the length (400-unit edge)', r.longShifts.length >= 3 && r.longShifts.every((v) => within(v, 1.8, 0.5)), JSON.stringify(r.longShifts))
check('stretching an edge moves motes continuously, phase untouched', r.stretchPhaseSame && r.stretch.length >= 3 && r.stretch.every((v) => Math.abs(v) <= 1.5), JSON.stringify(r.stretch))
check('syncing another edge leaves existing motes in place', r.syncKeepsMotes <= 2, r.syncKeepsMotes)
check('motes fog out by 800 units, present at 100', r.motesAt800 === 0 && r.motesAt100 > 20, JSON.stringify([r.motesAt800, r.motesAt100]))
check('hovered edge motes take the hover colour', r.hoverMotesWarm)
check('edge through the camera plane: trimmed, drawn ahead only, nothing stray', r.trim.low > 100 && r.trim.high === 0 && r.trim.off === 0, JSON.stringify(r.trim))
check('widths are CSS px: twice the device-px strength at pixel ratio 2', within(r.ratioSums[1] / r.ratioSums[0], 2, 0.2), JSON.stringify(r.ratioSums))
check('3000 edges: 2 draw calls on layer 0 (edges + motes)', r.callsLayer0 === 2, r.callsLayer0)
check('3000 edges: drift update < 0.5 ms/frame', r.updateMs3000 < 0.5, r.updateMs3000)
