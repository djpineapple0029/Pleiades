...
const res = await page.evaluate(async () => {
  ...
  const time = (frames = 60) => {
    for (let i = 0; i < 5; i++) {
      view.update((clock += 0.016), camera)
      bloom.render()
    }
  ...
  const labels = (on) => {
    for (const n of graph.nodes.values()) {
      if (!texts.has(n.id)) texts.set(n.id, n.label)
      n.label = on ? texts.get(n.id) : ''
    }
  }
  const cpu = () => {
    const t0 = performance.now()
    for (let i = 0; i < 60; i++) view.update((clock += 0.016), camera)
  ...
  const measure = (name) => {
    ...
    out[name] = {
      frameMs: +a.toFixed(2),
      noLabelsMs: +b.toFixed(2),
      labelsMs: +(a - b).toFixed(2),
      layoutCpuMs: +(ca - cb).toFixed(3),
      shown: view.labelsShown().length,
    }
  }
  const settle = () => {
    view.sync()
