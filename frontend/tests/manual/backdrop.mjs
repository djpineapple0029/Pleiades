...
const frames = await page.evaluate(async () => {
  ...
  const shoot = (name) => {
    ...
    for (const n of graph.nodes.values()) {
      ...
      stars.push({
        ...
        x,
        y,
        rPx: (r * pxPerUnit) / depth,
        size: r / 3,
        dist,
        shown: shown.has(n.id),
      })
    }
  ...
  for (let i = 0; i < 7; i++) {
    chain.push(graph.addNode({ x: i * 70, y: (i % 2) * 25, z: 0, label: names[i] }).id)
    if (i) graph.addEdge(chain[i - 1], chain[i])
  }
  ...
  for (let c = 0; c < CLUSTERS; c++) {
    ...
    for (let i = 0; i < PER; i++) {
      const n = graph.addNode({
        ...
        y: oy + (rand() - 0.5) * 120,
        z: oz + (rand() - 0.5) * 120,
        label: i === 0 ? `Topic ${String.fromCharCode(65 + c)}` : title(),
      })
  ...
  for (let i = 0; i < 24; i++) {
    const a = ids[Math.floor(rand() * CLUSTERS)],
      b = ids[Math.floor(rand() * CLUSTERS)]
