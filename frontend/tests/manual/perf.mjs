...
const res = await page.evaluate(async () => {
  ...
  const measure = (name) => {
    ...
    let a = Infinity,
      b = Infinity
    for (let k = 0; k < 3; k++) {
      a = Math.min(a, time(single))
      b = Math.min(b, time(bloom.render))
    }
    ...
    dust.visible = true
    out[name] = {
      singlePassMs: +a.toFixed(2),
      bloomPipelineMs: +b.toFixed(2),
      overheadMs: +(b - a).toFixed(2),
      skyAndDustMs: +(a - noSky).toFixed(2),
    }
  }
  measure('empty map')
  const n0 = graph.addNode({ x: 0, y: 0, z: 0 })
  view.sync()
  for (const [name, d] of [
    ['one star filling the screen (d=15)', 15],
    ['one core star filling the screen', 45],
    ['one star at spawn distance (d=90)', 90],
  ]) {
    graph.setCore(n0.id, name.includes('core'))
    view.sync()
    camera.position.set(d * 0.3, d * 0.2, d)
    camera.lookAt(0, 0, 0)
    measure(name)
  }
  ...
  physics.start()
  let g = 0
  while (physics.isRunning && g++ < 3000) physics.update()
  let cx = 0,
    cy = 0,
    cz = 0
  ...
  measure('180-node map, inside it')
  ...
  for (let i = 0; i < 3000; i++)
    graph.addNode({ x: (rand() - 0.5) * 1500, y: (rand() - 0.5) * 1500, z: (rand() - 0.5) * 1500 })
  view.sync()
  ...
  camera.lookAt(0, 0, 0)
  measure('3000 nodes, flying inside')
