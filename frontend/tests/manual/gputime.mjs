...
page.on('pageerror', (e) => console.log('page error:', e.message)); page.on('console', (m) => { if (m.text().startsWith('[q]')) console.log(m.text()) })
...
const res = await page.evaluate(async () => {
  ...
  at('    renderer.setClearColor(clearColor, clearAlpha)', '    window.__q.end(); window.__q.begin("downsample")')
  at('    for (let i = 1; i < LEVELS; i++) pass(downsample, levels[i - 1], levels[i])', '    window.__q.end(); window.__q.begin("upsample")')
  at('    for (let i = LEVELS - 2; i >= 0; i--) pass(upsample, levels[i + 1], levels[i])', '    window.__q.end(); window.__q.begin("canvas")')
  at('    // 4.', '    window.__q.end(); window.__q.begin("composite")', true)
  src = src.replace('    renderer.autoClear = autoClear\n  }', '    renderer.autoClear = autoClear\n    window.__q.end()\n  }')
  ...
  for (const aa of [true, false]) {
    ...
    const renderer = new THREE.WebGLRenderer({ canvas, antialias: aa }); renderer.setSize(W, H, false)
    ...
    let current = null
    window.__q = {
      begin(name) { const q = gl.createQuery(); gl.beginQuery(ext.TIME_ELAPSED_EXT, q); current = { q, name } },
      end() { if (!current) { console.log('[q] end without begin', new Error().stack.split('\n').slice(1, 4).join(' | ')); return } gl.endQuery(ext.TIME_ELAPSED_EXT); pending.push(current); current = null },
    }
    ...
    graph.addNode({ x: 0, y: 0, z: 0 }); view.sync()
    camera.position.set(0, 0, 90)
    const bloom = mod.createBloom(renderer, scene, camera)
    const single = () => { window.__q.begin('singlePass'); camera.layers.enable(1); renderer.render(scene, camera); camera.layers.set(0); window.__q.end() }
    const frames = 60
    for (let i = 0; i < frames; i++) { view.update(1 + i * 0.016); bloom.render(); single() }
    gl.finish()
    ...
    for (let tries = 0; tries < 200 && pending.length; tries++) {
      ...
      for (let i = pending.length - 1; i >= 0; i--) {
        ...
        if (!gl.getParameter(ext.GPU_DISJOINT_EXT)) (totals[name] ??= []).push(gl.getQueryParameter(q, gl.QUERY_RESULT) / 1e6)
        gl.deleteQuery(q); pending.splice(i, 1)
      }
    }
    const row = {}
    for (const [name, xs] of Object.entries(totals)) { xs.sort((a, b) => a - b); row[name] = +xs[Math.floor(xs.length / 2)].toFixed(3) }
    row.pipeline = +(['stars', 'downsample', 'upsample', 'canvas', 'composite'].reduce((s, k) => s + (row[k] ?? 0), 0)).toFixed(3)
