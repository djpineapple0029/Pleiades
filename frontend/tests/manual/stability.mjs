...
const res = await page.evaluate(async (configs) => {
  ...
  const renderer = new THREE.WebGLRenderer({ canvas, antialias: true }); renderer.setPixelRatio(1); renderer.setSize(W, H, false)
  const gl = renderer.getContext()
  const scene = new THREE.Scene()
  const camera = new THREE.PerspectiveCamera(70, W / H, 0.5, 20000); scene.add(camera)
  const graph = createGraph(); const view = createGraphView(graph, scene, renderer); view.update(1)
  ...
  for (const cfg of configs) {
    ...
    for (const [dist, path] of [[855,'x'],[855,'d'],[1400,'x'],[1400,'d']]) {
      camera.position.set(0, 0, dist); camera.updateMatrixWorld(true)
      const unitPx = (H / 2 / Math.tan(35 * Math.PI / 180)) / dist
      const sums = [], stars = [], lo = new Float32Array(W * H).fill(1e9), hi = new Float32Array(W * H).fill(-1e9)
      for (let s = 0; s < 17; s++) {
        node.x = (s / 8) / unitPx; node.y = path === 'd' ? (s / 11) / unitPx : 0; view.syncNodes()
        bloom.render(); const a = read(); zero.render(); const b = read()
        let t = 0, u = 0; for (let i = 0; i < a.length; i++) if ((i & 3) !== 3) { t += a[i] - b[i]; u += b[i] }
        sums.push(t); stars.push(u)
        for (let p = 0; p < W * H; p++) { const x = p % W, y = (p / W) | 0; const rr = Math.hypot(x - W / 2, y - H / 2); if (rr < 4 * 5 * unitPx + 3) continue
          const v = a[p * 4] + a[p * 4 + 1] + a[p * 4 + 2] - b[p * 4] - b[p * 4 + 1] - b[p * 4 + 2]; lo[p] = Math.min(lo[p], v); hi[p] = Math.max(hi[p], v) }
      }
      ...
      let swing = 0; for (let p = 0; p < W * H; p++) if (hi[p] > -1e9) swing = Math.max(swing, hi[p] - lo[p])
      out.push({ cfg: JSON.stringify(cfg).slice(0, 30), dist, path, swing, starPx: +(5 * unitPx).toFixed(2), mean: Math.round(m), cv: +cv.toFixed(3), starCV: +cvs.toFixed(3) })
    }
    bloom.dispose()
  }
  return out
