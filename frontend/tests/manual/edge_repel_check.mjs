...
const out = await page.evaluate(async () => {
  ...
  const settle = (physics, cap = 6000) => { let n = 0; while (physics.isRunning && n++ < cap) physics.update(); return n }
  ...
  function tangleReport(graph) {
    ...
    const pos = (id) => { const n = graph.getNode(id); return { x: n.x, y: n.y, z: n.z } }
    const gaps = []
  ...
  // === Scenario 1: small dense "friend group", too-close-example.png shape ===
  {
    ...
    const ids = names.map((label) => graph.addNode({ x: (rand() - 0.5) * 40, y: (rand() - 0.5) * 40, z: (rand() - 0.5) * 40, label }).id)
    const [me, tor, rinya, arthur, freddy, merek, alyssa, romy] = ids
    const pairs = [
      [me, tor], [me, rinya], [me, freddy],
      [tor, rinya], [tor, arthur], [tor, freddy], [tor, merek],
      [rinya, arthur], [rinya, freddy],
      [arthur, freddy], [arthur, alyssa],
      [freddy, merek], [freddy, alyssa],
      [alyssa, romy], [merek, freddy],
    ]
    ...
    const c = ids.reduce((acc, id) => { const n = graph.getNode(id); acc.x += n.x; acc.y += n.y; acc.z += n.z; return acc }, { x: 0, y: 0, z: 0 })
    c.x /= ids.length; c.y /= ids.length; c.z /= ids.length
    let radius = 0
    for (const id of ids) { const n = graph.getNode(id); radius = Math.max(radius, Math.hypot(n.x - c.x, n.y - c.y, n.z - c.z)) }
    camera.position.set(c.x, c.y, c.z + radius * 3.2 + 40)
    camera.up.set(0, 1, 0)
    camera.lookAt(c.x, c.y, c.z)
    camera.updateMatrixWorld()
    for (let i = 0; i < 40; i++) { r._t = (r._t ?? 0) + 0.016; view.update(r._t, camera) }
    pipeline.render()
  ...
  // === Scenario 2: multi-cluster map, separation must not regress ===========
  {
    ...
    const view = createGraphView(graph, scene, renderer)
    const physics = createPhysics(graph, view)
    const CLUSTERS = 6, PER = 30
    const groups = []
    for (let c = 0; c < CLUSTERS; c++) {
      const ox = (rand() - 0.5) * 500, oy = (rand() - 0.5) * 500, oz = (rand() - 0.5) * 500
      const group = []
      for (let i = 0; i < PER; i++) {
        const n = graph.addNode({ x: ox + (rand() - 0.5) * 100, y: oy + (rand() - 0.5) * 100, z: oz + (rand() - 0.5) * 100 })
    ...
    for (let i = 0; i < 14; i++) {
      const a = groups[Math.floor(rand() * CLUSTERS)], b = groups[Math.floor(rand() * CLUSTERS)]
    ...
    const centroid = (ids) => { const c = { x: 0, y: 0, z: 0 }; for (const id of ids) { const n = graph.getNode(id); c.x += n.x; c.y += n.y; c.z += n.z }; return { x: c.x / ids.length, y: c.y / ids.length, z: c.z / ids.length } }
    ...
    for (let i = 0; i < CLUSTERS; i++) for (let j = i + 1; j < CLUSTERS; j++) minApart = Math.min(minApart, dist(cents[i], cents[j]))
    r.minApart = minApart
    ...
    let radius = 0
    for (const id of all) { const n = graph.getNode(id); radius = Math.max(radius, Math.hypot(n.x - mid.x, n.y - mid.y, n.z - mid.z)) }
  ...
  // === Scenario 3: cost at 3000 nodes, tick budget must not regress =========
  {
    ...
    const physics = createPhysics(graph, view)
    const ids = []
    for (let i = 0; i < 3000; i++) ids.push(graph.addNode({ x: (rand() - 0.5) * 3000, y: (rand() - 0.5) * 3000, z: (rand() - 0.5) * 3000 }).id)
    for (let c = 0; c < 60; c++) {
      const members = ids.slice(c * 50, c * 50 + 50)
      for (let i = 0; i < members.length; i++) for (let j = 0; j < 4; j++) graph.addEdge(members[i], members[(i + 1 + j) % members.length])
    ...
    let ticks = 0
    for (let f = 0; f < 40; f++) { physics.update(); ticks++ }
